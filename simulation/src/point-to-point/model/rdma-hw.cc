...
TypeId RdmaHw::GetTypeId (void)
{
	static TypeId tid = TypeId ("ns3::RdmaHw")
		...
		.AddAttribute("TimelyMinRtt",
				...
				MakeUintegerChecker<uint64_t>())
		.AddAttribute("TimelyMinRttTemp",
				"MinRtt of TIMELY (ns) Temp",
				UintegerValue(20000),
				MakeUintegerAccessor(&RdmaHw::m_tmly_minRtt_tmp),
				MakeUintegerChecker<uint64_t>())
		.AddAttribute("TimelyMinRttUpdateCount",
				"update min rtt every 10 times of rtt",
				UintegerValue(0),
				MakeUintegerAccessor(&RdmaHw::m_tmly_cnt),
				MakeUintegerChecker<uint32_t>())
...
int RdmaHw::ReceiveUdp(Ptr<Packet> p, CustomHeader &ch){
	...
	if (x == 1 || x == 2){ //generate ACK or NACK
		...
		seqh.SetTimeStamp(Simulator::Now().GetTimeStep());
...
void RdmaHw::UpdateNextAvail(Ptr<RdmaQueuePair> qp, Time interframeGap, uint32_t pkt_size){
	...
	// printf("sendingTime: %lu\n",sendingTime);
	// std::cout << "sending Time" << sendingTime << "\n";
...
void RdmaHw::UpdateRateDCI(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch, bool fast_react){
	...
	if(qp->tmly.m_lastUpdateSeq == 0){ // not first RTT
		if (qp->DCI.m_ackCnt ==2) {
			...
			// printf("sip: %u,dip: %u, already_send: %u send nxt: %u, TargetRate = %.3lf\n",qp->sip.Get(), qp->dip.Get(), qp->snd_una, qp->snd_nxt, qp->lastPktSize, qp->DCI.m_gap, new_gap, qp->m_baseRtt, TargetRate.GetBitRate()*1e-9);
	...
	// fout<< Simulator::Now().GetTimeStep()-2020000000 << " " << std::fixed <<qp->m_rate.GetBitRate()*1e-9<<std::endl;
	// fout.close();
}
...
void RdmaHw::UpdateRateTimely(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch, bool us){
	...
	// DataRate TargetRate;
	// m_tmly_cnt++;
	// if (print && m_node->GetId() == 2)
		// printf("seq:%u next_seq:%u node:%u, m_tmly_cnt=%u, minRTT=%lu rtt=%lu\n", qp->tmly.m_lastUpdateSeq, next_seq, m_node->GetId(), m_tmly_cnt, m_tmly_minRtt, rtt);
	// if (rtt < m_tmly_minRtt_tmp)
	// 	m_tmly_minRtt_tmp = rtt;
	// if (m_tmly_cnt >= 10)
	// {
	// 	m_tmly_cnt = 0;
	// 	// m_tmly_minRtt = m_tmly_minRtt_tmp;
	// }
	// qp->DCI.m_ackCnt++;
	if (qp->tmly.m_lastUpdateSeq != 0){ // not first RTT
	// 	if (qp->DCI.m_ackCnt ==2) {
	// 		qp->DCI.m_gap = ack_ts - qp->DCI.m_last_ts;
	// 	} else if (qp->DCI.m_ackCnt <= 10){
	// 		uint64_t new_gap = ack_ts - qp->DCI.m_last_ts;
	// 		qp->DCI.m_gap = (1 - m_tmly_alpha) * qp->DCI.m_gap + m_tmly_alpha * new_gap;
	// 		double elstimate_bw = double(next_seq - qp->tmly.m_lastUpdateSeq) / double(qp->DCI.m_gap);
	// 		double cwnd = elstimate_bw * rtt / double(qp->lastPktSize);
	// 		printf("ackcnt: %u, gap: %lu, Estimate BW: %lf, rate_now: %.3lf, cwnd: %lf,\n",qp->DCI.m_ackCnt, qp->DCI.m_gap, elstimate_bw, qp->tmly.m_curRate.GetBitRate() * 1e-9, cwnd);
	// 	}else if(qp->DCI.m_ackCnt > 10){
	// 		uint64_t new_gap = ack_ts - qp->DCI.m_last_ts;
	// 		qp->DCI.m_gap = (1 - m_tmly_alpha) * qp->DCI.m_gap + m_tmly_alpha * new_gap;
	...
	// 	}
		...
		// if (print)
		double cwnd_now = qp->m_rate.GetBitRate() * 1e-9 * rtt / qp->lastPktSize;
			// printf(" %c %.3lf, cwnd: %.3lf\n", inc? '^':'v', qp->m_rate.GetBitRate() * 1e-9, cwnd_now);
	}
 ...
 *********************/
 ...
 void RdmaHw::UpdateRateTimely_M(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch, bool us){
	...
	if (rtt < m_tmly_minRtt)
		m_tmly_minRtt = rtt;
	// if (m_node->GetId() == 7)
	// printf("node:%u, minRTT=%lu rtt=%lu\n", m_node->GetId(), m_tmly_minRtt, rtt);
	if (qp->tmly.m_lastUpdateSeq!= 0){ // not first RTT
		int64_t new_rtt_diff = (int64_t)rtt - (int64_t)qp->tmly.lastRtt;
		double rtt_diff = (1 - m_tmly_alpha) * qp->tmly.rttDiff + m_tmly_alpha * new_rtt_diff;
		double gradient = rtt_diff / m_tmly_minRtt_tmp;
		bool inc = false;
		double c = 0;
		if (qp->tmly.avgRtt!=0){
			qp->tmly.avgRtt = (1-m_tmly_alpha) * rtt + m_tmly_alpha * qp->tmly.avgRtt;
		}else{
			qp->tmly.avgRtt = rtt;
		}
		uint64_t rtt_target = m_tmly_minRtt*1.5; // 后续改成单独的参数
		// if (print)
		...
		if (rtt <= rtt_target){
			...
			inc = false;
			c = 1 - m_tmly_beta * rtt_diff/rtt_target;
			if (c < 0)
				c = 0;
		}
		...
		if (inc){
			if (qp->tmly.m_incStage < 5){
				qp->m_rate = qp->tmly.m_curRate+(1-m_tmly_alpha)*(rtt_target-qp->tmly.avgRtt)/rtt_target;
			}else{
				qp->m_rate = qp->tmly.m_curRate + m_rhai;
			}
			if (qp->m_rate > qp->m_max_rate)
